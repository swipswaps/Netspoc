...
#          for each domain.
# Comment: NAT domains get different partition ID, if they belong to
...
sub find_subnets_in_nat_domain {
    ...
    for my $network (@networks) {
        ...
        my $id = $network->{zone}->{nat_domain}->{partition};
...
# {default_addr}.
# Store other addresses in {nat_addr}, separately for each nat_set.
sub setup_printable_addresses {
    my ($nat_domains, $nat_partiton) = @_;
    for my $network (@networks) {
        next if $network->{ip} =~ /^(?:unnumbered|tunnel)$/;
        ...
        if ($nat_hash) {
            ...
            for my $tag (sort keys %$nat_hash) {
                my $nat = $nat_hash->{$tag};
                next if $nat->{hidden};
                my @active = grep { $_->{nat_set}->{$tag} } @domains;
                $all_nat += @active;
                if (@active > $max_nat) {
                    $max_nat = @active;
                    $max_tag = $tag;
        ...
        if ($max_tag) {
            ...
            @domains = grep { not $_->{nat_set}->{$max_tag} } @domains;
        }
        else {
        ...
        if ($nat_hash) {
            ...
            for my $tag (keys %$nat_hash) {
                ...
                for my $domain (@domains) {
                    ...
                    $nat_attr->{$nat_set} = $addr;
                    $set_nat_obj->($nat_set, $tag, $nat, $addr);
                }
            }

            # Find NAT domains, where original address is visible,
            # i.e. no NAT tags are set in nat_set.
            if ($max_tag) {
                for my $domain (@domains) {
                    my $nat_set = $domain->{nat_set};
                    my $inactive = grep { not $nat_set->{$_} } keys %$nat_hash;
                    if ($inactive == keys %$nat_hash) {
                        $nat_attr->{$nat_set} = $orig_addr;
                        $set_orig_obj->($nat_set);
                    }
                }
            }
            $network->{nat_addr} = $nat_attr if $nat_attr;
        }                    
    }
}
        ...
        my $process = sub {
            ...
            if ($need_protect and $protect_self) {
                $acl->{need_protect} = [
                    ...
                    unique
                    map({ $_->{nat_addr}->{$nat_set} || $_->{default_addr} }
                        @$need_protect) ];
            }
            ...
            for my $what (qw(intf_rules rules)) {
                ...
                for my $rule (@$rules) {
                    ...
                    $new_rule->{src} =
                        [ map { 
                            $_->{nat_addr}->{$nat_set} || $_->{default_addr} 
                          }
                          @{ $rule->{src} } ];
                    $new_rule->{dst} =
                        [ map {  
                            $_->{nat_addr}->{$dst_nat_set} || $_->{default_addr}
                          }
                          @{ $rule->{dst} } ];
                    $new_rule->{prt} =
                        $rule->{printed_prt} ||=
