...
func genReverseRules1(rules []*groupedRule) []*groupedRule {
	...
	for _, rule := range rules {
		...
		if !hasStatelessRouter { continue }
		...
		for _, prt := range newPrtGroup {
			...
			case "udp":
				...
				if prt.ports != arefTcpAny {
					newSrcRange = prt
				}
				newPrt = rule.srcRange || prtUdp.dstRange
			default: // proto == "ip"
				newPrt = prt
			}
